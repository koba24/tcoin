...
The RPM spec file provided here is for Tcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://tcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Tcoin git repository](https://github.com/tcoin/tcoin) but are not part of
...
Currently only `Source10` is used, for the example `tcoin.conf` file.
...
in the tcoin git repository. At present only an SVG version of the Tcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the tcoin
...
needed when 0.12.1 is released, a proper fix is already in the Tcoin git
master branch.
...
idea to build Tcoin Core against a static version of that library compiled
...
The `tcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `tcoin-server` package. At this point, most RPM based
...
The `tcoin-server` package should have SELinux support. How to properly do
...
Tcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/tcoin/). He has been
packaging Tcoin for Fedora at least since 2012.
...
4. I renamed the `tcoin` package that contains the Qt GUI to `tcoin-core` as
that appears to be how the general population refers to it, in contrast to
`tcoin-xt` or `tcoin-classic`. I wanted to make sure the general population
