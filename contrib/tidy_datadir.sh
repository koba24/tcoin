#!/bin/bash
# Copyright (c) 2013 The Tcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Tcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Tcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Tcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Tcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Tcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Tcoin 0.8 datadir."
    ;;
esac
